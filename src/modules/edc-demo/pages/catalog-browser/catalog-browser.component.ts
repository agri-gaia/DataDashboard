    ...
    ngOnInit(): void {
        ...
        this.authenticationService.userProfile$.subscribe(userProfile => {
            ...
            this.route.data.subscribe(data => {
                ownAssets = data.ownAssets
            })

            this.url = userProfile.url;

            this.filteredContractOffers$ = this.apiService.getContractOffers(this.url, ownAssets);
        })

        this.fetch$
            .pipe(debounceTime(300), skip(1))
            .subscribe((searchTerm) => this.filteredContractOffers$ = this.apiService.getFilteredContractOffers(searchTerm, this.url, ownAssets));
    }
